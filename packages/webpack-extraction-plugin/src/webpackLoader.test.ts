...
describe('webpackLoader', () => {
  ...
  it('should throw an error for invalid hash salt', () => {
    expect(() => validateHashSalt(SOURCE_CODE, 'invalid-salt')).toThrowErrorMatchingInlineSnapshot(
      `[Error: GriffelCSSExtractionPlugin: classNameHashSalt is set to "invalid-salt", but the salt location comment contains "salt". Please ensure that all files use the same salt.]`,
    );
    expect(() => validateHashSalt(SOURCE_CODE, 'sal')).toThrowErrorMatchingInlineSnapshot(
      `[Error: GriffelCSSExtractionPlugin: classNameHashSalt is set to "sal", but the salt location comment contains "salt". Please ensure that all files use the same salt.]`,
    );
  });
  ...
  it('should throw if "sourceCode" does not contain a comment', () => {
    expect(() =>
      ...
      `[Error: GriffelCSSExtractionPlugin: classNameHashSalt is set to "salt", but no salt location comment was found in the source code.]`,
    );
  });
});
