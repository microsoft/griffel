...
import { applyStyles, BLACK, BLUE, getColor, RED, render, resetBrowserTestState } from '../testing/browserHelpers.js';
...
describe('@scope boundary isolation', () => {
  test('descendant inside scope gets scoped styles; past boundary does not', () => {
    ...
    expect(getColor(scoped)).toBe(RED);
    expect(getColor(outside)).toBe(BLACK);
  });
  ...
  test('nested scope root re-establishes scope past the boundary', () => {
    ...
    expect(getColor(inside)).toBe(BLUE);
    expect(getColor(past)).toBe(BLACK);
    expect(getColor(restart)).toBe(BLUE);
  });
});
