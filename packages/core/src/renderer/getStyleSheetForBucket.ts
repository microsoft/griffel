...
export function getStyleSheetForBucket(
  ...
  const containerMeta = metadata['c'] as string | undefined;
  const container = containerMeta ?? '0';
  ...
  if (!renderer.stylesheets[stylesheetKey]) {
    ...
    const stylesheet = createIsomorphicStyleSheet(tag, bucketName, priority, {
      ...
      ...(isMediaBucket && { media }),
      ...(isContainerBucket && containerMeta !== undefined && { [DATA_CONTAINER_ATTR]: containerMeta }),
    });
...
function isSameInsertionKey(
  ...
  const targetCondition = (metadata['m'] as string | undefined) ?? (metadata['c'] as string | undefined) ?? '';
  const elementCondition = element.media || element.getAttribute(DATA_CONTAINER_ATTR) || '';

  const targetKey = bucketName + targetCondition;
  const elementKey = element.getAttribute(DATA_BUCKET_ATTR) + elementCondition;
