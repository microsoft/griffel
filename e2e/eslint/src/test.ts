...
async function performTest() {
  ...
  try {
    ...
    console.error('❌', 'ESLint should have reported an error for the hook-naming violation, but it did not.');
    process.exit(1);
  } catch (e) {
    const output = (e as Error).message;

    if (output.includes('@griffel/hook-naming')) {
      console.log('✅', 'ESLint with flat config correctly reported @griffel/hook-naming violation');
    } else {
      console.error('❌', 'ESLint failed but not with the expected @griffel/hook-naming error:');
